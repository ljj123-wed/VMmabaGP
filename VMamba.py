...
from openstl.modules.simvp_modules import SpatialAttention,TemporalAttention
...
class VMamba(nn.Module):
    def __init__(
        ...
        self.attn = SpatialAttention(hidden_dim, 21)
        ...
        self.layer_scale_1 = nn.Parameter(1e-2 * torch.ones((hidden_dim)), requires_grad=True)
        if self.mlp_branch:
    ...
    def forward(self, input: torch.Tensor):
        if self.flag:
            ...
            input = input + self.drop_path(self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * self.attn(self.norm3(input)))
